...
<main class="container py-4" style="max-width:1100px">
    <div class="d-flex justify-content-between align-items-start mb-3"><div><h1 class="h4 mb-1">Dlouhodobé názvy kroužků</h1><p class="text-muted mb-0">Opravte název a popis nebo archivujte kroužek, který už nemá otevřené nabídky.</p></div><a class="btn btn-outline-secondary btn-sm" href="club_programs_admin.php">Programy a podmínky</a></div>
    <div class="alert alert-info"><strong>Tato stránka neobsahuje přednastavené hodnoty.</strong> Položky jako „500+1“ nebo kód „K“ jsou skutečně uložené názvy a interní kódy vzniklé při dřívějším zadání. Název můžete opravit níže; interní kód se kvůli vazbám a auditu nemění. Konkrétní cena, termín a kapacita se nastavují v nabízeném období.</div>
    ...
    <?php foreach ($programs as $program): ?>
        <div class="card border-0 shadow-sm mb-3"><div class="card-header bg-white d-flex justify-content-between"><span><strong><?=programSettingsH($program['name'])?></strong> <span class="small text-muted">interní kód</span> <code><?=programSettingsH($program['code'])?></code></span><span class="badge text-bg-<?=$program['status']==='active'?'success':'secondary'?>"><?=programSettingsH($program['status'])?></span></div><div class="card-body"><div class="small text-muted mb-3">Celkem nabízených období: <strong><?=(int)$program['offer_count']?></strong> · otevřených nebo rozpracovaných: <strong><?=(int)$program['open_offer_count']?></strong></div>
