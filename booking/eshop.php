...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrf_verify((string)($_POST['csrf_token'] ?? ''))) $errors[] = 'Formulář vypršel. Obnovte stránku.';
        ...
        if ($action === 'add') {
            $variantId = (int)($_POST['variant_id'] ?? 0);$offer = false;
...
foreach (shopStorefrontCatalog($pdo) as $product) {
    $isProgram = clubProgramProductHasOfferLink($pdo, (int)$product['product_id']);
    $product['variants'] = array_values(array_filter(
        ...
        static function (array $variant) use ($pdo, $isProgram): bool {
            ...
            return $isProgram
                ? clubProgramVariantSaleState($pdo,(int)$variant['variant_id'])['saleable']
                : $offer === false;
        }
    ));
...
$activeCatalogCount = count($products);
...
<div class="row g-4"><section class="col-lg-8"><form method="get" class="card border-0 shadow-sm mb-3" role="search"><div class="card-body row g-2 align-items-end"><?php if($selectedCategory!==''):?><input type="hidden" name="kategorie" value="<?=shopPublicH($selectedCategory)?>"><?php endif;?><div class="col-md-7"><label class="form-label" for="storefront-search">Hledat v názvu a popisu</label><input class="form-control" id="storefront-search" name="q" value="<?=shopPublicH($catalogSearch)?>"></div><div class="col-md-3"><label class="form-label" for="storefront-sort">Řazení</label><select class="form-select" id="storefront-sort" name="razeni"><option value="order" <?=$catalogSort==='order'?'selected':''?>>doporučené pořadí</option><option value="name" <?=$catalogSort==='name'?'selected':''?>>podle názvu</option><option value="price" <?=$catalogSort==='price'?'selected':''?>>od nejnižší ceny</option></select></div><div class="col-md-2 d-grid"><button class="btn btn-primary">Použít</button></div></div></form><nav class="mb-3" aria-label="Kategorie produktů"><div class="d-flex flex-wrap gap-2 align-items-center"><a class="btn btn-sm <?=$selectedCategory===''?'btn-primary':'btn-outline-primary'?>" href="eshop.php">Vše</a><?php foreach($categoryMenu as$category):?><a class="btn btn-sm <?=$selectedCategory===$category['category_path']?'btn-primary':'btn-outline-primary'?>" href="eshop.php?kategorie=<?=rawurlencode((string)$category['category_path'])?>" title="<?=shopPublicH((string)($category['description']??''))?>"><?php if((int)$category['depth']>0):?><span aria-hidden="true"><?=str_repeat('↳ ',(int)$category['depth'])?></span><?php endif;?><?=shopPublicH($category['display_name'])?> <span class="badge text-bg-light"><?=(int)$category['product_count']?></span></a><?php endforeach;?></div></nav><div class="d-flex flex-wrap justify-content-between align-items-center gap-2"><h2 class="h5 mb-0"><?=$selectedCategory===''?'Aktivní nabídka':'Kategorie: '.shopPublicH($selectedCategoryLabel)?></h2><span class="small text-muted" aria-live="polite"><?=count($products)?> <?=count($products)===1?'výsledek':'výsledků'?></span></div><div class="row g-3 mt-0">
<?php foreach ($products as $product): $variantCount=count($product['variants']);$imageUrl=shopStorefrontPrimaryImageUrl($product['images']); ?><div class="col-md-6"><div class="card h-100 border-0 shadow-sm"><img src="<?=shopPublicH($imageUrl)?>" alt="<?=shopPublicH($product['public_name'])?>" class="card-img-top app-product-image p-2" style="height:220px" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='<?=shopPublicH(shopStorefrontPlaceholderImageUrl())?>'"><div class="card-body d-flex flex-column"><div class="d-flex justify-content-between gap-2"><h3 class="h6"><?=shopPublicH($product['public_name'])?></h3><?php if ($product['is_program']): ?><span class="badge text-bg-primary align-self-start">kroužek</span><?php endif;?></div><p class="small text-muted"><?=nl2br(shopPublicH($product['public_summary']))?></p><?php if($product['program_restrictions']!==[]):?><div class="small text-primary mb-1"><?=shopPublicH(implode(' · ',$product['program_restrictions']))?></div><?php endif;?><?php if($product['listing_attributes']!==[]):?><div class="d-flex flex-wrap gap-1 mb-2"><?php foreach($product['listing_attributes']as$attribute):?><span class="badge text-bg-light border"><?=shopPublicH($attribute['display_name'].': '.$attribute['formatted_value'])?></span><?php endforeach;?></div><?php endif;?><div class="small text-muted"><?=$variantCount?> <?=$variantCount===1?'varianta':($variantCount<=4?'varianty':'variant')?></div><div class="d-flex justify-content-between align-items-center mt-auto pt-3"><strong><?php if ((int)$product['min_amount_minor'] !== (int)$product['max_amount_minor']): ?>od <?php endif;?><?=shopPublicMoney((int)$product['min_amount_minor'],(string)$product['currency'])?></strong><a class="btn btn-sm btn-primary" href="produkt.php?id=<?=(int)$product['product_id']?>"><?=$product['in_stock']?'Detail a varianty':'Zobrazit detail'?></a></div></div></div></div><?php endforeach;?>
<?php if ($products === []): ?><div class="col-12"><div class="alert alert-light border mb-0"><?php if($activeCatalogCount>0):?><h3 class="h6">Žádný produkt neodpovídá zvoleným filtrům.</h3><p class="mb-2"><?php if($catalogSearch!==''):?>Hledaný výraz: <strong><?=shopPublicH($catalogSearch)?></strong>. <?php endif;?>V katalogu je <?= $activeCatalogCount ?> aktivních nabídek.</p><a class="btn btn-outline-primary" href="eshop.php">Zrušit filtry</a><?php else:?><h3 class="h6">Aktuálně není zveřejněná žádná nabídka.</h3><p class="mb-2">Můžete si prohlédnout klubové akce nebo se vrátit později.</p><a class="btn btn-outline-primary" href="krouzky.php">Akce</a><?php endif;?></div></div><?php endif;?></div></section>
