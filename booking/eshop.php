...
<?php if ($products === []): ?><div class="col-12"><div class="alert alert-light border mb-0"><?php if($activeCatalogCount>0):?><h3 class="h6">Žádný produkt neodpovídá zvoleným filtrům.</h3><p class="mb-2"><?php if($catalogSearch!==''):?>Hledaný výraz: <strong><?=shopPublicH($catalogSearch)?></strong>. <?php endif;?>V katalogu je <?= $activeCatalogCount ?> aktivních nabídek.</p><a class="btn btn-outline-primary" href="eshop.php">Zrušit filtry</a><?php else:?><h3 class="h6">Aktuálně není zveřejněná žádná nabídka.</h3><p class="mb-2">Můžete si prohlédnout klubové akce nebo se vrátit později.</p><a class="btn btn-outline-primary" href="krouzky.php">Kroužky a akce</a><?php endif;?></div></div><?php endif;?></div></section>
