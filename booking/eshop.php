...
$selectedCategory=trim((string)($_GET['kategorie']??''));$categoryCounts=[];
foreach($products as$product)foreach($product['categories']as$category)$categoryCounts[$category]=($categoryCounts[$category]??0)+1;
uksort($categoryCounts,static fn(string$a,string$b):int=>strcasecmp($a,$b));
if($selectedCategory!=='')$products=array_values(array_filter($products,static fn(array$product):bool=>in_array($selectedCategory,$product['categories'],true)));
