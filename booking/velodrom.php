...
require_once dirname(__DIR__) . '/includes/public_velodrome.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify((string)($_POST['csrf_token'] ?? ''))) {
        ...
        } catch (InvalidArgumentException | PublicVelodromeException $exception) {
...
<div class="row g-3 mb-4"><?php foreach($slots as $slot):?><div class="col-md-6"><section class="card shadow-sm border-0 h-100"><div class="card-body"><div class="d-flex justify-content-between"><h2 class="h5"><?=publicVelodromeH($slot['nazev'])?></h2><span class="badge text-bg-<?=$slot['remaining_capacity']>0?'success':'secondary'?>"><?=$slot['remaining_capacity']?> / <?=$slot['effective_capacity']?> volno</span></div><p class="mb-1"><strong><?=publicVelodromeH($slot['datum'])?></strong>, <?=publicVelodromeH(substr((string)$slot['cas_od'],0,5))?>–<?=publicVelodromeH(substr((string)$slot['cas_do'],0,5))?></p><p class="small text-muted"><?=$slot['public_exclusive_booking']?'Výhradní rezervace celého velodromu':'Sdílená hodina'?> · <?=number_format((float)$slot['cena_kc'],0,',',' ')?> Kč</p><?php if($profile&&$slot['remaining_capacity']>0):?><form method="post" class="row g-2"><?=csrf_field()?><input type="hidden" name="action" value="reserve"><input type="hidden" name="lesson_id" value="<?=(int)$slot['id']?>"><div class="col-8"><input class="form-control form-control-sm" name="note" maxlength="1000" placeholder="Poznámka (nepovinná)"></div><div class="col-4 d-grid"><button class="btn btn-primary btn-sm">Rezervovat</button></div></form><?php endif;?></div></section></div><?php endforeach;?><?php if($slots===[]):?><div class="col-12"><div class="alert alert-secondary">Není vypsána žádná veřejná hodina velodromu.</div></div><?php endif;?></div>
<section class="card shadow-sm border-0"><div class="card-header bg-white fw-semibold">Moje rezervace velodromu</div><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Termín</th><th>Účastník</th><th>Stav</th><th></th></tr></thead><tbody><?php foreach($reservations as $reservation):?><tr><td><?=publicVelodromeH($reservation['datum'].' '.substr((string)$reservation['cas_od'],0,5).'–'.substr((string)$reservation['cas_do'],0,5))?></td><td><?=publicVelodromeH($reservation['prijmeni'].' '.$reservation['jmeno'])?></td><td><?=publicVelodromeH($reservation['stav'])?><?=$reservation['zaplaceno']?' · zaplaceno':''?></td><td><?php if(in_array($reservation['stav'],['ceka','potvrzena'],true)):?><form method="post" class="d-flex gap-2"><?=csrf_field()?><input type="hidden" name="action" value="cancel"><input type="hidden" name="reservation_id" value="<?=(int)$reservation['id']?>"><input class="form-control form-control-sm" name="note" maxlength="1000" required placeholder="Důvod storna"><button class="btn btn-outline-danger btn-sm">Zrušit</button></form><?php endif;?></td></tr><?php endforeach;?><?php if($reservations===[]):?><tr><td colspan="4" class="text-center text-muted py-3">Zatím nemáte rezervaci.</td></tr><?php endif;?></tbody></table></div></section>
</main></body></html>
