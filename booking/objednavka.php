...
require_once dirname(__DIR__).'/db.php';require_once dirname(__DIR__).'/csrf_helper.php';require_once dirname(__DIR__).'/includes/shop_checkout.php';require_once dirname(__DIR__).'/includes/stripe_gateway.php';
...
try{
    ...
    if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
        if(!csrf_verify((string)($_POST['csrf_token']??'')))throw new StripeGatewayException('Formulář vypršel. Obnovte stránku.');
        if((string)($_POST['action']??'')!=='stripe_checkout')throw new StripeGatewayException('Neplatná platební akce.');
...
}catch(StripeGatewayException|InvalidArgumentException $exception){
    $stripeError=$exception->getMessage();$qr=$order['payment_record_status']==='pending'?shopPaymentQrDataUri((string)$order['spd_payload']):null;
}
$stripeAvailable=stripeIsEnabled()&&$order['status']==='placed'&&$order['payment_record_status']==='pending';
$messages=[
...
?>
<!doctype html>
<html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Objednávka <?=orderPublicH($order['public_code'])?></title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"><?php appUiAssets(); ?></head>
...
<?php if($stripeError!==''):?><div class="alert alert-danger"><?=orderPublicH($stripeError)?></div><?php endif;?>
