...
if(in_array('--help',$arguments,true)){
    echo "Použití: APP_HOST=<host> php bin/expire-shop-orders.php [--apply] [--limit=N] [--now='Y-m-d H:i:s'] [--json]\n";
    echo "Bez --apply se nic nemění (dry-run). --now je určeno pro testy a ruční obnovu, ne pro běžný cron.\n";
    exit(0);
}
...
if($unknown!==[]||!is_string($host)||$host===''||preg_match('/^[a-z0-9.-]+(?::\d+)?$/Di',$host)!==1){
    fwrite(STDERR,"Neplatné argumenty nebo chybí APP_HOST. Použijte --help.\n");exit(64);
}
...
}catch(Throwable $exception){fwrite(STDERR,'Expirace objednávek selhala: '.$exception->getMessage().PHP_EOL);exit(1);}
