...
?>
<!doctype html>
<html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Objednávky</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous"><link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet" integrity="sha384-tViUnnbYAV00FLIhhi3v/dWt3Jxw4gZQcNoSCxCIFNJVCx7/D55/wXsrNIRANwdD" crossorigin="anonymous"></head>
<body class="bg-light"><?php include __DIR__.'/hlavicka.php';?>
<main class="container-fluid py-4" style="max-width:1550px">
<div class="d-flex justify-content-between align-items-center mb-3"><div><h1 class="h4 mb-0"><i class="bi bi-receipt me-2 text-primary"></i>Objednávky</h1><div class="small text-muted">Na jednom místě zjistíte, co připravit, vydat nebo stornovat.</div></div><details class="text-end"><summary class="btn btn-outline-secondary btn-sm">Pokročilé</summary><div class="card card-body position-absolute end-0 mt-2 shadow-sm" style="z-index:10"><a href="eshop_order_expiry_admin.php" class="btn btn-sm btn-link text-start">Ruční expirace nezaplacených</a><a href="eshop_notifications_admin.php" class="btn btn-sm btn-link text-start">Fronta e-mailů</a></div></details></div>
...
<div class="alert alert-info small">Tady řešíte zboží a zákazníka. Skutečný pohyb peněz potvrzuje pozice <strong>Hospodář a platby</strong>; potřebný úkol se jí zobrazí automaticky.</div>
...
<?php foreach($orders as$order):?><?php $items=$orderItems[(int)$order['id']]??[];$isProgramOrder=in_array('program',array_column($items,'line_type'),true);?><tr id="order-<?=(int)$order['id']?>">
<td><code><?=orderAdminH($order['public_code'])?></code><div class="small text-muted"><?=orderAdminH($order['created_at'])?></div></td>
<td><?=orderAdminH($order['customer_name_snapshot'])?><div class="small"><?=orderAdminH($order['customer_email_snapshot'])?></div><?php if(($order['checkout_mode']??'account')==='guest'):?><span class="badge text-bg-light border mt-1">bez registrace</span><?php elseif(($order['checkout_mode']??'')==='quick_program'):?><span class="badge text-bg-primary mt-1">rychlá přihláška</span><?php endif;?></td>
...
<td><strong><?=orderAdminH($isProgramOrder&&$order['status']==='processing'?'Zaplaceno / přihláška':orderAdminStatusLabel((string)$order['status']))?></strong><?php if(!$isProgramOrder&&$order['ready_at']!==null):?><div class="small text-muted">Připraveno <?=orderAdminH($order['ready_at'])?></div><?php endif;?><?php if(!$isProgramOrder&&$order['completed_at']!==null):?><div class="small text-muted">Vydáno <?=orderAdminH($order['completed_at'])?></div><?php endif;?><?php if($order['cancelled_at']!==null):?><div class="small text-muted">Storno <?=orderAdminH($order['cancelled_at'])?></div><?php endif;?></td>
...
<td>
<?php if($order['status']==='processing'&&!$isProgramOrder):?>
...
<?php elseif($order['status']==='ready'&&!$isProgramOrder):?>
...
<?php elseif($order['status']==='processing'&&$isProgramOrder):?><div class="alert alert-info py-2 small">Kroužek se nevydává. Po schválení údajů sportovce se účast automaticky propojí se soupiskou.</div><?php endif;?>
...
<tr class="table-light"><td colspan="7" class="p-3"><strong class="small text-uppercase text-muted"><?=$isProgramOrder?'Obsah přihlášky':'Co přesně připravit'?></strong>
