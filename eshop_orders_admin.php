...
<div class="d-flex justify-content-between align-items-center mb-3"><div><h1 class="h4 mb-0"><i class="bi bi-receipt me-2 text-primary"></i>Objednávky K4</h1><div class="small text-muted">Platba, příprava, osobní výdej a auditované storno se skladovou kompenzací.</div></div><div class="d-flex gap-2"><a href="eshop_order_expiry_admin.php" class="btn btn-outline-warning btn-sm">Expirace nezaplacených</a><a href="eshop_admin.php" class="btn btn-outline-secondary btn-sm">Administrace e-shopu</a></div></div>
...
<form method="post" class="border border-warning rounded p-2 mb-2"><?=csrf_field()?><input type="hidden" name="action" value="confirm_refund"><input type="hidden" name="order_id" value="<?=(int)$order['id']?>"><label class="small fw-semibold">Potvrdit odeslanou vratku</label><input class="form-control form-control-sm my-1" name="refund_reference" maxlength="255" required aria-label="Reference vratky <?=orderAdminH($order['public_code'])?>" placeholder="Reference bankovní transakce"><input class="form-control form-control-sm my-1" name="reason" maxlength="1000" required aria-label="Poznámka k vratce <?=orderAdminH($order['public_code'])?>" placeholder="Jak a kým byla vratka ověřena"><label class="small d-block"><input type="checkbox" name="confirm_action" value="1" required> Peníze byly skutečně odeslány</label><button class="btn btn-sm btn-warning mt-1">Potvrdit vratku</button></form>
<?php endif;?>
