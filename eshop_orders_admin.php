...
function orderAdminPaymentLabel(string $status):string{return ['pending'=>'Čeká','paid'=>'Zaplaceno','cancelled'=>'Zrušeno','refund_required'=>'Vrátit platbu'][$status]??$status;}
...
<td><?=orderAdminMoney((int)$order['total_minor'],(string)$order['currency'])?><div><code>VS <?=orderAdminH($order['variable_symbol'])?></code></div></td>
...
<td><span class="badge text-bg-<?=in_array($order['payment_record_status'],['paid'],true)?'success':($order['payment_record_status']==='refund_required'?'danger':'warning')?>"><?=orderAdminH(orderAdminPaymentLabel((string)$order['payment_record_status']))?></span></td>
...
<?php elseif(in_array($order['status'],['completed','cancelled'],true)):?><span class="small text-muted">Objednávka je v koncovém stavu.</span><?php endif;?>
