...
<?php if($order['payment_record_status']==='refund_required'):?><div class="border border-warning-subtle bg-warning-subtle rounded p-2 mb-2"><strong class="small d-block">Objednávka je stornovaná. Peníze ještě nebyly vráceny.</strong><?php if(staffCanUsePosition('finance_manager')):?><form method="post" action="prepnout_pracovni_pozici.php" class="mt-2"><?=csrf_field()?><input type="hidden" name="position" value="finance_manager"><input type="hidden" name="next" value="eshop_payments_admin.php"><input type="hidden" name="reason" value="Dokončení vratky objednávky <?=orderAdminH($order['public_code'])?>"><button class="btn btn-sm btn-warning">Přejít k vrácení platby</button></form><?php else:?><span class="small">Úkol dokončí uživatel v pracovní pozici Hospodář a platby.</span><?php endif;?></div><?php endif;?>
