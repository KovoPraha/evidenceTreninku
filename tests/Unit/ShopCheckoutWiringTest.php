...
final class ShopCheckoutWiringTest extends TestCase
{
    ...
    public function testAdminBankConfirmationRequiresRoleCsrfReasonAndConfirmation():void
    {
        ...
        self::assertStringContainsString("roleAtLeast('admin')",$source);self::assertStringContainsString('csrf_verify',$source);self::assertStringContainsString("(\$_POST['confirm_action']??'')==='1'",$source);self::assertStringContainsString('shopOrderAdminConfirmBankPayment',$source);self::assertStringContainsString('shopOrderAdminCancel',$source);self::assertStringContainsString('shopOrderAdminMarkReady',$source);self::assertStringContainsString('shopOrderAdminCompletePickup',$source);self::assertStringContainsString('reason',$source);
    }
