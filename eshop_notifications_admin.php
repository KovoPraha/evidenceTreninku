...
?>
<!doctype html><html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Fronta e-mailů K3</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"><link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet"></head><body class="bg-light">
<?php include __DIR__ . '/hlavicka.php'; ?><main class="container-fluid py-4" style="max-width:1450px"><div class="d-flex justify-content-between align-items-center mb-3"><div><h1 class="h4 mb-0"><i class="bi bi-envelope-exclamation me-2 text-primary"></i>Fronta oznámení K3</h1><div class="small text-muted">Provozní přehled neodeslaných e-mailů po povýšení z čekací listiny.</div></div><a href="eshop_admin.php" class="btn btn-outline-secondary btn-sm">Administrace e-shopu</a></div>
...
<div class="card border-0 shadow-sm"><div class="card-header bg-white d-flex justify-content-between"><strong><?=notificationAdminH($status===''?'Neodeslaná oznámení':'Filtr: '.$status)?></strong><a href="eshop_notifications_admin.php" class="btn btn-sm btn-outline-secondary">Neodeslaná</a></div><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Stav</th><th>Akce / dítě</th><th>Příjemce</th><th>Pokusy</th><th>Dostupné</th><th>Poslední chyba</th><th>Ruční opakování</th></tr></thead><tbody><?php foreach($rows as $row):?><tr><td><span class="badge text-bg-<?=$row['status']==='failed'?'danger':($row['status']==='sent'?'success':($row['status']==='processing'?'info':'warning'))?>"><?=notificationAdminH($row['status'])?></span></td><td><strong><?=notificationAdminH($row['event_name'])?></strong><div class="small text-muted"><?=notificationAdminH($row['child_last_name'].' '.$row['child_first_name'])?></div></td><td><?=notificationAdminH($row['recipient_email'])?></td><td><?=(int)$row['attempts']?></td><td><?=notificationAdminH($row['available_at'])?></td><td class="small"><?=notificationAdminH($row['last_error']??'')?></td><td><?php if(in_array($row['status'],['failed','pending'],true)):?><form method="post" class="d-flex flex-column gap-1" style="min-width:250px"><?=csrf_field()?><input type="hidden" name="notification_id" value="<?=(int)$row['id']?>"><input class="form-control form-control-sm" name="reason" maxlength="1000" required placeholder="Důvod ručního opakování"><label class="small"><input type="checkbox" name="confirm_retry" value="1" required> Potvrzuji nové zařazení</label><button class="btn btn-sm btn-outline-primary">Vrátit do fronty</button></form><?php else:?><span class="text-muted small">Nelze během zpracování ani po odeslání</span><?php endif;?></td></tr><?php endforeach;?><?php if($rows===[]):?><tr><td colspan="7" class="text-center text-muted py-4">V tomto filtru nejsou žádná oznámení.</td></tr><?php endif;?></tbody></table></div></div></main><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script></body></html>
