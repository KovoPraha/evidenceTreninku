...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!csrf_verify((string)($_POST['csrf_token']??'')))$errors[]='Formulář vypršel. Obnovte stránku.';
        ...
        if(($_POST['confirm_expire']??'')!=='1')throw new InvalidArgumentException('Expiraci je nutné výslovně potvrdit.');
        $ids=array_values(array_unique(array_filter(array_map('intval',(array)($_POST['order_ids']??[])),static fn(int$id):bool=>$id>0)));
        if($ids===[])throw new InvalidArgumentException('Vyberte alespoň jednu objednávku z náhledu.');
        $expired=0;$unchanged=0;
        foreach($ids as$id){$result=shopOrderExpirePending($pdo,$id,$now,true);$result['changed']?$expired++:$unchanged++;}
        $_SESSION['flash_expiry']='Expirováno '.$expired.' objednávek; beze změny '.$unchanged.'.';header('Location: eshop_order_expiry_admin.php',true,303);exit;
...
<div class="alert alert-warning small">Potvrzení použije stejný transakční storno proces: zruší platební předpis, právě jednou vrátí sklad a uvolní drženou kapacitu velodromu. Každá objednávka se před změnou znovu zamkne a ověří.</div>
<form method="post"><?=csrf_field()?><div class="card border-0 shadow-sm"><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th></th><th>Objednávka</th><th>Zákazník</th><th>Částka</th><th>Splatnost</th><th>Velodrom</th></tr></thead><tbody>
...
<?php if($orders===[]):?><tr><td colspan="6" class="text-center text-muted py-4">Žádná nezaplacená objednávka po splatnosti.</td></tr><?php endif;?></tbody></table></div></div>
<?php if($orders!==[]):?><div class="border border-danger-subtle rounded bg-white p-3 mt-3"><label class="d-block"><input type="checkbox" name="confirm_expire" value="1" required> Potvrzuji expiraci vybraných nezaplacených objednávek a uvolnění jejich rezervací.</label><button class="btn btn-outline-danger mt-2">Expirovat vybrané</button></div><?php endif;?></form>
</main></body></html>
