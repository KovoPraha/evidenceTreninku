...
/** @return array{program_cancellation:array{id:int,text:string},program_consent:array{id:int,text:string>} */
function clubCatalogImportTermTemplates(PDO $pdo):array
{
    ...
    foreach(CLUB_PROGRAM_TERM_PURPOSES as$purpose){
        ...
        if(!$row||trim((string)$row['consent_text_plain'])==='')throw new ClubCatalogImportException('Chybí schválený produkční vzor podmínek: '.$purpose.'.');
        $result[$purpose]=['id'=>(int)$row['id'],'text'=>(string)$row['consent_text_plain']];
    }
    return$result;
}
...
/** @return array{created:int,updated:int,programs:int,products:int,variants:int} */
function clubCatalogImport(PDO$pdo,int$actorId,string$applicationRoot):array
{
    ...
    foreach(legacyClubCatalog()as$index=>$row){
        ...
        if(!$program){
            ...
            $result=clubProgramWizardCreate($pdo,$actorId,[
                ...
                'program_cancellation_source'=>'existing','program_cancellation_version_id'=>$templates['program_cancellation']['id'],
                'program_consent_source'=>'existing','program_consent_version_id'=>$templates['program_consent']['id'],
        ...
        foreach(CLUB_PROGRAM_TERM_PURPOSES as$purpose)if(!clubProgramTermsCurrent($pdo,'program',$programId,$purpose))clubProgramTermsConfigure($pdo,$actorId,'program',$programId,$purpose,$templates[$purpose]['text'],true);
    ...
    return['created'=>$created,'updated'=>$updated,'programs'=>(int)$pdo->query("SELECT COUNT(*) FROM club_programs WHERE code LIKE 'KROUZKY-2627-%'")->fetchColumn(),'products'=>(int)$pdo->query("SELECT COUNT(DISTINCT product_id) FROM club_program_offers o JOIN club_programs p ON p.id=o.program_id WHERE p.code LIKE 'KROUZKY-2627-%'")->fetchColumn(),'variants'=>(int)$pdo->query("SELECT COUNT(*) FROM club_program_offers o JOIN club_programs p ON p.id=o.program_id WHERE p.code LIKE 'KROUZKY-2627-%'")->fetchColumn()];
}
