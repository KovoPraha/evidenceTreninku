...
{
    return$pdo->query("SELECT v.id,v.product_id,v.sku,v.stock_quantity_decimal,p.name AS product_name FROM shop_variants v JOIN shop_products p ON p.id=v.product_id WHERE p.offer_type<>'program' ORDER BY p.name,v.sku,v.id")->fetchAll(PDO::FETCH_ASSOC);
}
