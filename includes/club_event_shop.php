...
{
    $public=clubEventColumnExists($pdo,'club_events','planning_status')&&clubEventColumnExists($pdo,'club_events','visibility')?" AND e.planning_status='confirmed' AND e.visibility='public'":'';
    $rows=$pdo->query("SELECT e.*,v.id AS variant_id,v.sku,v.amount_minor FROM club_events e JOIN shop_product_event_links l ON l.event_id=e.id JOIN shop_products p ON p.id=l.product_id JOIN shop_variants v ON v.product_id=p.id WHERE e.event_type IN ('club_event','camp') AND e.status='open'".$public." AND e.pricing_policy='product_variants' AND p.catalog_status='active' AND v.catalog_status='active' AND v.price_mode='fixed' AND v.amount_minor>0 AND v.currency='CZK' AND (v.visible IS NULL OR v.visible=1) AND (e.registration_starts_at IS NULL OR e.registration_starts_at<=CURRENT_TIMESTAMP) AND (e.registration_ends_at IS NULL OR e.registration_ends_at>=CURRENT_TIMESTAMP) AND EXISTS (SELECT 1 FROM club_event_sessions s WHERE s.event_id=e.id AND s.status='scheduled' AND s.ends_at>=CURRENT_TIMESTAMP) ORDER BY e.registration_starts_at,e.id,v.id")->fetchAll(PDO::FETCH_ASSOC);
